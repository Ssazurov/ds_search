...
- **Баг «вкладка не запоминается» (2026-09-24), фикс `db58ead`:** изначальная реализация читала `?tab=` в Python (`active_name`/`active_idx`), но `st.tabs()` не умеет открывать вкладку по индексу программно — эти переменные никак не влияли на рендер, и JS только писал вкладку в URL при клике, но не восстанавливал её при загрузке. Добавлено восстановление: при старте JS ищет в URL `tab=`, сравнивает с `data-baseweb="tab"` по индексу и, если активная вкладка не совпадает, эмулирует `.click()` по нужной; обработчик клика вынесен раньше и защищён флагом `parent.window.__dsTabSyncBound`, чтобы не плодиться при каждом Streamlit-rerun. Контейнер пересобран, `curl localhost:8501` → HTTP 200.
