...
- **Баг «вкладка не запоминается» (2026-09-24), фикс `db58ead`, добор `1f1ca0d`:** изначальная реализация читала `?tab=` в Python (`active_name`/`active_idx`), но `st.tabs()` не умеет открывать вкладку по индексу программно — эти переменные никак не влияли на рендер, и JS только писал вкладку в URL при клике, но не восстанавливал её при загрузке (`db58ead`). После этого фикса восстановление всё ещё не срабатывало именно на полном F5 (а не на Streamlit-rerun): iframe-компонент синхронизации грузится независимо от основного DOM и мог выполниться раньше, чем React отрисует кнопки `[data-baseweb="tab"]`. `1f1ca0d`: восстановление ждёт появления кнопок (poll до ~3с, `tryRestore`), убрано двойное URL-кодирование (`encodeURIComponent` поверх `URLSearchParams.set`, который сам кодирует), удалена неиспользуемая `TABS_BY_NAME`. Контейнер пересобран, `curl localhost:8501` → HTTP 200; ручная проверка в браузере (клик по вкладке → F5) не проводилась — Claude Desktop не имеет визуального доступа к браузеру.
