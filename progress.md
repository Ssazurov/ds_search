...
## 2026-09-08 -- issue #27 (ds_ingestion): найден баг идемпотентности state

Прогон 27/100 (докат остатка downsideup в GAR) был прерван вручную -
data/downsideup.ingested.json оказался пустым (0 записей), т.к.
run_adapter сохранял state только один раз в конце цикла, а не после
каждого документа. Без фикса повторный запуск задублировал бы уже
отправленные 27 документов в GAR. Серверного дедупа по
source_fingerprint/document_key в gar-core-api клиенте нет -
единственный механизм идемпотентности это локальный state-файл
(doc_id = sha256(source_url)).

Фикс: PR Ssazurov/ds_ingestion#1 (_save_state вызывается инкрементально
после каждого успешного ingest_document).

Не выполнено: реальный докат остатка (73/100) - gar-core-api
