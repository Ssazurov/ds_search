...
async def _save_pdf(pdf_url: str, teaser_url: str, domain: str, direction: str,
                     license_result, out_dir: Path) -> dict | None:
    pdf_url = urljoin(teaser_url, pdf_url)
    doc_id = doc_id_for(teaser_url)
...
async def download_single(source: dict, data_root: Path = DEFAULT_DATA_ROOT) -> dict:
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if is_pdf_teaser_page(html):
            ...
            if pdf_url:
                meta = await _save_pdf(pdf_url, canon, domain, direction, license_result, out_dir)
                if meta:
                    return meta
